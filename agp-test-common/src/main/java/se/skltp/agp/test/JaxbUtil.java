...
public class JaxbUtil {
	...
	public JaxbUtil(Class... classesToBeBound) {
        try {
        	if (log.isDebugEnabled()) log.debug("Load JAXBContext based on classes: " + Arrays.toString(classesToBeBound));
    ...
    public void setContextPath(String contextPath) {
        try {
        	if (contextPath == null || contextPath.length() == 0) {
            	if (log.isDebugEnabled()) log.debug("No context path, let's wait with creating the jaxbContext");
        		
        	} else {
            	if (log.isDebugEnabled()) log.debug("Load JAXBContext based on context path: " + contextPath);
	...
	public String marshal(Object jaxbObject) {
    	...
    	try {
            ...
            if (log.isDebugEnabled()) log.debug("marshalled jaxb object of type {}, returns xml: {}",  jaxbObject.getClass().getSimpleName(), xml);
	...
	public Object unmarshal(Object payload) {
		...
		// Precondition, check that the jaxbContext is set!
    	if (jaxbContext == null) {
				log.error("Trying to unmarshal with a null jaxbContext, returns null. Check your configuration, e.g. jaxb-transformers!");
    		return null;
    	}
    	...
    	try {
            ...
            if (jaxbObject instanceof JAXBElement) {
                if (log.isDebugEnabled()) log.debug("Found a JAXBElement, returns it value");
                jaxbObject = ((JAXBElement)jaxbObject).getValue();
            }

				log.debug("unmarshalled xml payload of type: {}, returns jaxb object of type {}", payload.getClass().getName(), jaxbObject.getClass().getName());
